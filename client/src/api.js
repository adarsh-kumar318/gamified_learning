const BASE_URL = import.meta.env.VITE_API_URL || 'https://levelup-learning-backend.onrender.com/api';
const TIMEOUT  = 15000;

/**
 * Helper to fetch with JWT token from localStorage.
 * Safely handles empty bodies, non-JSON error pages, and network failures.
 */
export const apiFetch = async (path, options = {}) => {
  ...
  try {
    ...
    // Server is down or request timed out
    throw new Error('Cannot reach the server. Is the backend running?');
  }

  // Read body as text first — avoids "Unexpected end of JSON input" on empty responses

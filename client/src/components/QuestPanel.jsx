...
import { saveQuestProgress, fetchQuestProgress, finalizeQuest, restartQuest, fetchQuestDetail, submitAnswerToQuest } from '../api';
...
export default function QuestPanel({ quest, onClose, onAnswer, onRefill, energy, userData }) {
  const isChallenge = quest.questions.length === 5;
  
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedOpt, setSelectedOpt] = useState(null);
  const [status, setStatus] = useState('loading'); // 'loading', 'idle', 'correct', 'wrong', 'complete', 'failed'
  ...
  const progress = (answers.length / quest.questions.length) * 100;

  if (status === 'loading') return (
    <div className="fixed inset-0 z-[100] bg-[#050510] flex items-center justify-center">
      <div className="text-accent2 animate-pulse font-title">Resuming Trial State...</div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="h-16 md:h-20 border-b border-white/5 bg-surface/50 backdrop-blur-xl px-4 md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div>
            ...
            <div className="font-title text-sm md:text-xl font-bold text-white leading-none truncate max-w-[150px] md:max-w-none">{quest.title}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
          ...
          {(isChallenge || quest.isBoss) && (
            <div className={`flex flex-col items-end px-3 md:px-4 py-1 md:py-1.5 rounded-xl border ${timeLeft < 20 ? 'bg-red-500/20 border-red-500 animate-pulse' : 'bg-white/5 border-white/10'}`}>
              <div className="text-[8px] md:text-[9px] font-black text-text3 uppercase">Time Remaining</div>
          ...
          <div className="hidden sm:flex flex-col items-end">
            ...
            <div className="text-xl font-black text-gold">+{earnedXp}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center overflow-hidden">
        {status === 'out-of-energy' ? (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button 
                ...
                className="px-12 py-5 bg-accent text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:scale-105 transition-all shadow-[0_0_30px_rgba(124,58,237,0.4)]"
              >
                🏮 Rest & Recover (10 Gold)
              </button>
              <div className="flex flex-col sm:flex-row gap-4">
              <button 
                onClick={onClose}
                className="px-12 py-5 bg-white/5 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-white/10 transition-all border border-white/10 hover:scale-105"
              >
                Return to Map 🗺️
              </button>
              <button 
                onClick={handleRestart}
                className="px-12 py-5 bg-red-600 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:scale-105 transition-all shadow-[0_0_30px_rgba(220,38,38,0.3)]"
              >
                Retry Attempt 🔄
              </button>
            </div>
          ...
          >
            ...
            <p className="text-text3 font-bold uppercase tracking-widest mb-10 text-sm md:text-base bg-black/40 backdrop-blur-sm px-4 py-2 rounded-lg">
              You has survived the trial and gained <span className="text-gold">{earnedXp} XP</span>
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                onClick={onClose}
                className="px-12 py-5 bg-gold text-black font-black uppercase tracking-[0.2em] rounded-2xl hover:scale-105 transition-all shadow-[0_0_30px_rgba(245,158,11,0.3)]"
              >
                Return to Map 🗺️
              </button>
              <button 
                onClick={handleRestart}
                className="px-12 py-5 bg-white/5 text-gold font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-white/10 transition-all border border-gold/30 hover:scale-105"
              >
                Replay Level 🔄
              </button>
            </div>
          ...
          >
            ...
            <button 
              onClick={onClose}
              className="px-16 py-6 bg-white/10 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-white/20 transition-all border border-white/20 hover:scale-105 active:scale-95"
            >
              Return to Map 🗺️
            </button>
          </motion.div>
          ...
          <div className="w-full max-w-3xl px-8 z-10">
            ...
            <div className="mb-8 md:mb-12">
              <div className="flex justify-between text-[11px] font-black text-text3 uppercase tracking-tighter mb-2">
                ...
                <span className="text-accent2">{Math.round(progress)}% Mastery</span>
              </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="text-[10px] font-black text-accent uppercase tracking-[0.3em] mb-4 opacity-70 text-center">TRIAL OBJECTIVE {currentIndex + 1}</div>
      ...
      {/* RPG Footer */}
      <div className="h-24 bg-surface/80 border-t border-white/5 backdrop-blur-xl flex items-center justify-center gap-12">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs font-bold text-text1">Patience is the warrior's shield.</div>
          </div>
        </div>
        <div className="w-px h-8 bg-white/5" />
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-xl">⚔️</div>
          <div>
            <div className="text-[10px] font-black text-text3 uppercase">Focus</div>
            <div className="text-xs font-bold text-text1">Precision over Speed.</div>
          </div>
        </div>
      </div>
    </motion.div>

...
const THEMES = {
  ...
  forest: {
    ...
    accentGlow: "drop-shadow-[0_0_30_rgba(34,197,94,0.6)]",
...
export default function RescueScene({ progress, status, timeLeft, avatarEmoji = "🧙", themeType = 'water' }) {
  ...
  const ParticleIcon = theme.particleIcon;
  const FailIcon = theme.failIcon;
      ...
      >
        ...
        {[...Array(12)].map((_, i) => (
          <motion.div
            ...
            animate={{ y: '-20%', opacity: [0, 0.8, 0], scale: [0.5, 1, 0.5] }}
            transition={{ duration: 2 + Math.random() * 2, repeat: Infinity, delay: Math.random() * 5 }}
            className={`absolute ${theme.labelColor} opacity-40`}
          >
            <ParticleIcon size={12 + Math.random() * 20} />
          </motion.div>
        ))}
      </div>
      ...
      >
        ...
        >
          <span className={`text-8xl block select-none ${theme.accentGlow}`}>
            {status === 'success' ? <Bird size={96} className="text-cyan-400" /> : avatarEmoji}
          </span>
          ...
          >
             <div className="text-center relative">
              <motion.div 
                ...
                className="text-gold mb-8 flex justify-center drop-shadow-[0_0_50px_rgba(245,158,11,0.5)]"
              >
                <Trophy size={110} fill="currentColor" />
              </motion.div>
        ...
        {status === 'failed' && (
          ...
          >
            <div className="text-center flex flex-col items-center">
              <div className={`mb-8 drop-shadow-[0_0_40px_rgba(0,0,0,0.5)] ${theme.labelColor} opacity-80`}>
                <FailIcon size={120} />
              </div>

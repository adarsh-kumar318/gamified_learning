/**
 * Seed script — Battle Questions
 * Run: node server/scripts/seedBattleQuestions.js
 *
...
async function seed() {
  try {
    ...
    // Upsert questions — won't duplicate on re-run
    let inserted = 0, skipped = 0;
    for (const q of QUESTIONS) {
      const exists = await BattleQuestion.findOne({ question: q.question });
      if (!exists) {
        await BattleQuestion.create(q);
        inserted++;
      } else {
        skipped++;
      }
